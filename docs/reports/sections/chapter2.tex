...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Trong chế độ NTK, MSE loss dẫn đến nghiệm closed-form dạng kernel regression, tạo nền tảng cho phân tích lý thuyết chính xác.
      \end{itemize}
...
Neural Tangent Kernel thực nghiệm (empirical NTK) được định nghĩa thông qua tích của ma trận Jacobian với chuyển vị của nó:
\begin{equation}
...
\begin{itemize}
    ...
    \item \textbf{Lớp ẩn:} Gồm $m$ nơ-ron với hàm kích hoạt ReLU, không sử dụng hệ số chệch (bias)
...
Việc không sử dụng hệ số chệch đơn giản hóa phân tích lý thuyết mà không làm mất tính tổng quát về mặt biểu diễn, vì hệ số chệch (bias) có thể được mô phỏng bằng cách thêm một chiều hằng số vào đầu vào.
...
Xét một bước tính toán của thuật toán với đầu vào được biểu diễn dưới dạng mẫu (template). Đầu ra của bộ dự đoán NTK có thể được phân tích thành hai thành phần:
\begin{equation*}
