...
\begin{itemize}
    ...
    \item \textbf{Hệ số chệch:} Mạng không sử dụng hệ số chệch trong các lớp
    \item \textbf{Chiều rộng lớp ẩn:} $n_h = 50,000$ nơ-ron ẩn, đủ lớn để tiệm cận chế độ NTK ở giới hạn chiều rộng vô hạn
\end{itemize}
...
\begin{itemize}
    \item \textbf{Thuật toán tối ưu:} Hạ gradient toàn bộ dữ liệu được sử dụng thay vì huấn luyện theo lô nhỏ, đảm bảo gradient được tính trên toàn bộ tập huấn luyện
    \item \textbf{Tốc độ học:} Được chọn đủ nhỏ để đảm bảo hội tụ ổn định trong chế độ huấn luyện lười
    \item \textbf{Tiêu chí dừng:} Huấn luyện cho đến khi hàm mất mát giảm đến ngưỡng đủ nhỏ hoặc đạt số epoch tối đa
\end{itemize}
...
\begin{itemize}
    \item \textbf{Tính chính xác:} Loại bỏ các yếu tố nhiễu từ quá trình huấn luyện (tốc độ học, số epoch, v.v.)
    \item \textbf{Tính hiệu quả:} Tính toán dạng tường minh thay vì tối ưu hóa lặp
...
Phần này trình bày các thực nghiệm huấn luyện thực tế để đánh giá khả năng học của mạng nơ-ron trong điều kiện thực tiễn.
...
So sánh số lượng mô hình thực tế cần thiết để đạt độ chính xác mục tiêu (90\%, 95\%, 99\%) với ngưỡng lý thuyết từ Công thức (8):
\begin{equation}
...
\begin{itemize}
    \item Khi kích thước ensemble tăng lên, độ chính xác tiến dần đến 100\%
...
Do sử dụng các bất đẳng thức theo trường hợp xấu nhất, ngưỡng này có tính chất \textbf{bảo thủ cao}. Trong thực tế:
\begin{itemize}
    \item Các phân phối thực tế thường ``tốt hơn'' giả định theo trường hợp xấu nhất
...
\begin{itemize}
    ...
    \item Phương pháp khớp mẫu với kỹ thuật ensemble là \textbf{khả thi về mặt tính toán}
