...
Tầm quan trọng của cấu trúc block-partitioned nằm ở việc nó tạo ra tính trực giao trong dữ liệu huấn luyện. Khi các mẫu thuộc các khối khác nhau trực giao với nhau, ma trận NTK có cấu trúc đặc biệt.
...
\begin{itemize}
    ...
    \item $\beta \mathbf{1}\mathbf{1}^\top$ là ma trận hạng 1
...
\subsection{Định lý 5.1: Bảo toàn thông tin dấu}
	...
	extbf{Định lý 5.1 (Hành vi bộ dự báo NTK):} Cho mạng nơ-ron hai lớp trong chế độ NTK ở giới hạn chiều rộng vô hạn được huấn luyện trên tập dữ liệu template $\mathcal{D}$. Gọi $\mu(\hat{\mathbf{x}})$ là kỳ vọng của phân phối giới hạn của đầu ra mạng tại điểm kiểm thử $\hat{\mathbf{x}}$. Khi đó, với mỗi bit đầu ra thứ $i$:
\begin{equation}
...
trong đó:
\begin{itemize}
    \item $w^1 > 0$ là \textit{trọng số tín hiệu}: Đóng góp từ template khớp chính xác
    \item $w^0$ là \textit{trọng số nhiễu}: Đóng góp từ các template có tương quan khác không
...
\begin{itemize}
    \item Template cho phép trừ: Tương tự phép cộng với số bù 2
...
Để đảm bảo xác suất lỗi không vượt quá $\delta$, số lượng mô hình $N$ cần thỏa mãn (Công thức (8) trong bài báo gốc):
\begin{equation}
