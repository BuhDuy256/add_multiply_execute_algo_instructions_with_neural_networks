...
\textbf{Hàm tổng hợp toàn cục:} Đầu ra của thuật toán được tổng hợp từ các hàm cục bộ thông qua phép tuyển theo bit:
\begin{equation}
...
\textbf{Định lý 5.1 (NTK Predictor Behavior):} Cho mạng nơ-ron hai lớp trong chế độ NTK (giới hạn chiều rộng vô hạn) được huấn luyện trên tập dữ liệu mẫu $\mathcal{D}$. Gọi $\mu(\hat{\mathbf{x}})$ là kỳ vọng của phân phối giới hạn của đầu ra mạng tại điểm test $\hat{\mathbf{x}}$. Khi đó, với mỗi bit đầu ra thứ $i$:
\begin{equation}
...
\textbf{Mã hóa mẫu cho SBN:} Bao gồm các thành phần:
\begin{itemize}
