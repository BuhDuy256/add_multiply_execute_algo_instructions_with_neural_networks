...
\textbf{Hàm tổng hợp toàn cục:} Đầu ra của thuật toán được tổng hợp từ các hàm cục bộ thông qua phép tuyển bitwise (bitwise disjunction):
\begin{equation}
...
\textbf{Định lý 5.1 (NTK Predictor Behavior):} Cho mạng nơ-ron hai lớp trong chế độ NTK (infinite-width limit) được huấn luyện trên tập dữ liệu template $\mathcal{D}$. Gọi $\mu(\hat{\mathbf{x}})$ là kỳ vọng của phân phối giới hạn của đầu ra mạng tại điểm test $\hat{\mathbf{x}}$. Khi đó, với mỗi bit đầu ra thứ $i$:
\begin{equation}
...
\textbf{Assumption 5.1:} Gọi $C_i = |U_i|$ là số lượng tương quan không mong muốn tại bit thứ $i$. Điều kiện để đầu ra có dấu đúng là:
\begin{equation}
    C_i < -\frac{w^1}{w^0}
\end{equation}
...
\textbf{Mã hóa template:} Mỗi bit được xử lý độc lập. Với bit thứ $i$, có 2 template:
\begin{itemize}
...
\textbf{Số lượng template:} $2l$ template tổng cộng.

\textbf{Tương quan không mong muốn:} Vì mỗi template chỉ phụ thuộc vào một bit đầu vào duy nhất và các bit được mã hóa trong các block riêng biệt, không có tương quan không mong muốn ($C_i = 0$).
...
\textbf{Giai đoạn 1 - Cộng bitwise:} Tính tổng từng bit và bit nhớ cục bộ:
\begin{align}
...
\textbf{Template cho mỗi vị trí:} Mỗi vị trí bit $i$ có $2^3 = 8$ template (tương ứng với 8 tổ hợp của $a_i, b_i, c_{i-1}$).

\textbf{Tương quan không mong muốn:} Tối đa $C_i = 1$, xảy ra khi hai template có cùng giá trị $(a_i, b_i)$ nhưng khác $c_{i-1}$.
...
\textbf{Số bước lặp:} $l$ bước cho $l$-bit multiplication.

\textbf{Mã hóa template:} Mỗi bước lặp được mã hóa thành các template riêng, bao gồm:
\begin{itemize}
...
\textbf{Mã hóa template cho SBN:} Bao gồm các thành phần:
\begin{itemize}
    \item Template cho phép trừ (subtraction): Tương tự phép cộng với số bù 2
    \item Template kiểm tra dấu kết quả
    \item Template cập nhật program counter
\end{itemize}
