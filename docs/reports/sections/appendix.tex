...
Định lý \textit{Sherman-Morrison} (1950) là công cụ quan trọng để tính nghịch đảo của ma trận khi có sự thay đổi hạng 1 (cập nhật hạng 1). Đây là nền tảng cho việc phân tích ma trận NTK có cấu trúc ``ma trận đơn vị có tỷ lệ + nhiễu hạng 1''.
...
trong đó $\mathbf{1}\mathbf{1}^\top$ là ma trận hạng 1. Áp dụng Sherman-Morrison với $A = \alpha I_M$, $u = v = \sqrt{\beta}\mathbf{1}$:
\begin{align}
...
Công thức này cho phép tính dạng tường minh của $\Theta^{-1}$ mà không cần thuật toán nghịch đảo ma trận $O(M^3)$, đồng thời làm rõ cấu trúc của bộ dự báo NTK.

\section{Các kết quả xác suất}

\subsection{Bổ đề tập trung Gaussian}

Bổ đề tập trung cho phân phối Gaussian là cơ sở để xác định số lượng mô hình trong tổ hợp cần thiết.
...
\end{equation}
	...
	extbf{Ứng dụng trong tính kích thước tổ hợp:}

Đầu ra của mỗi mô hình trong tổ hợp tuân theo phân phối Gaussian với kỳ vọng $\mu$ (có dấu đúng) và phương sai $\sigma^2$. Để đảm bảo trung bình tổ hợp $\overline{F}$ có dấu đúng với xác suất ít nhất $1 - \delta$, ta cần:
\begin{equation}
...
\begin{itemize}
    \item Tính toán dạng tường minh của nhân NTK cho các kiến trúc mạng phổ biến
...
\begin{itemize}
    ...
    \item \texttt{templates/multiplication.py}: Mẫu cho thuật toán dịch và cộng
